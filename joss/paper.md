...
## Determining whether $k$ $n$-balls intersect in $n$-dimensional space

The *Čech complex* of a point cloud is an abstract simplicial complex which is homotopy equivalent
to the union of balls of radius $r$ centered on each point in the cloud. For $k$ ranging from $0$ to 
the dimension of the space, $n$, a $k$-simplex generated by $k+1$ points in the cloud is part of the 
complex if the $n$-balls of radius $r$ centered on each point have a common intersection. 
Equivalently, the $k$-simplex is part of the complex if the points can be contained in a sphere of 
radius $r$.

Determining whether a 1-simplex exists is trivial: the distance between the two points comprising
them must be less than or equal to $r$ to include that simplex. However, for simplices generated by
three or more points, determining whether the intersection between the balls exists is less
straightforward. However, it can be implemented with the *conformal geometric algebra* (CGA) of $n$ 
dimensions, the Clifford algebra $\text{Cl}_{n+1,1,0}\left(\mathbb{R}\right)$. Conformal geometric
algebras generalize two-dimensional compass-and-straightedge constructions of Euclidean geometry to
an arbitrary number of dimensions.

In order to determine whether the intersection exists, we need to convert each point in the point
cloud to a CGA representation of an $n$-sphere of radius $r$. To do this, we identify 1-blades of
CGA with *round points*: points centered on a coordinate with an associated radius. This task can be
accomplished with the following function. This function is only defined for `StaticVector` instances
 from the StaticArrays.jl package in order to preserve type stability.
```julia
using CliffordNumbers

function cga_ball(point::StaticVector{D}, radius::Real)
    p2 = sum(abs2, point)
    r2 = sign(radius) * radius^2    # imaginary radius is meaningful in CGA, but not relevant yet
    return KVector{1,CGA(D)}(p2 - r2 + 1, p2 - r2 - 1, 2*point...)
end
```
Next, we must find the intersection of the spheres, which can be done by taking the wedge product 
(in this context, referred to as the *meet*) of the spheres. As an example, the meet of two round 
points is the circle formed by the intersection of the spheres, if they do intersect. Note that the 
wedge product of more 1-blades than the dimension of the algebra is zero, so without modification, 
this only works for a maximum of $k = n+1$ balls.

Every $k$-blade in CGA has an associated radius, and its square can be calculated by dividing the
squared norm of the blade by the squared norm of the left contraction of the vector at infinity with
the input, which corresponds to the weight of the blade in this homogeneous model.
```julia
function squared_radius(k::KVector{K,Q}) where {K,Q}
    # Generate the point at infinity
    n∞ = KVector{1,Q}(1, 1, ntuple(Returns(0), Val(dimension(Q) - 2)))
    return abs2(k) / abs2(n∞ ⨼ k)
end
```
Once the intersection is constructed, we can determine its squared radius. In CGA, the radius of an
object can be real or imaginary, corresponding to positive or negative squares of the radius,
respecitvely. A real radius corresponds to touching spheres, and an imaginary radius corresponds to
spheres that do not touch. If the radius (and thus the squared radius) is zero, then the spheres are
tangent to each other. 

The final function is given below. This particular implementation can be optimized further, and it
treats tangent spheres as intersecting (this may be changed by altering the `>=` operator to `>`).
```julia
function do_spheres_intersect(radius::Real, points::StaticVector...)
    balls = cga_ball.(points, radius)
    intersection = reduce(∧, balls)
    return squared_radius(intersection) >= 0
end
```
