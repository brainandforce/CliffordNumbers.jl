...
Figures accompanying the examples were generated with Makie.jl.
...
function CGA_sphere(point::StaticVector{D}, radius) where D
    ...
    return x + 1//2 * (sum(point.^2) - radius^2) * ninf
end

function CGA_squared_radius(x::KVector{K,Q}) where {K,Q}
    Q isa CGA || error("input blade must be an element of a CGA.")
    ninf = KVector{1,Q}(1, 1, zero(SVector{dimension(x) - 2})...)
    return x * grade_involution(x) * left_contraction(ninf, x)^-2
end
\end{lstlisting}
...
\begin{flalign}
s = \left( \left(r I^{-1}\right) \wedge n_\infty \right) \vee r
\end{flalign}
...
\end{lstlisting}
This facilitates constructing a sphere that passes through less than $n+1$ points.
...
Specifically, this includes Steven de Keninck (enki), developer of ganja.js, Hamish Todd, Moab Croft (Slackline), and David Cohoe (sudgylacmoe), who produces educational videos about geometric algebra, including the video that introduced him to the field, \textit{A Swift Introduction to Geometric Algebra}.
