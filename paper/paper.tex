...
Most literature on Clifford algebras uses matrix-based formalisms
The work of David Hestenes, 
...
While 3D rotations and reflections can be composed and performed with matrix-vector multiplication, it has suboptimal efficiency, is not numerically stable, and can introduce shearing that does not respect the orthonormality of the space.

CliffordNumbers.jl aims to provide an implementation of geometric algebras that is fast and easy to use, with a well-documented implementation that can be used as a reference for others who wish to implement the fundamental operations of geometric algebra.
While it supports algebras of arbitrary dimension, the most common anticipated use cases are small 
...
Each versor corresponds to an isometry of space, with $1$-versors corresponding to reflections, $2$-versors to rotations, and $n$-versors to inversions.
...
\subsection{Direct and mirror interpretations}
...
These algebras are not isomorphic, but their even subalgebras are APS.
