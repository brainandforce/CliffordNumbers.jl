...
!!! note "`det` on a bare subtype"
    [`det`](@ref CliffordNumbers.det) on a large algebra (`2ⁿ > 32`) reduces an explicit
    left-multiplication matrix built from run-time blade arithmetic, with no generated kernel, so it
    accepts a bare subtype value. The geometric product is always generated and has no run-time
    fallback, which is why the `Signature` conversion is needed for products.
