...
@inline function to_index(C::Type{<:KVector{K,Q}}, b::BitIndex{Q}) where {K,Q}
    # Default to 0 as a flag that we didn't find the desired index
    i = 0
    for (n,a) in enumerate(Tuple(BitIndices(C)))
        is_same_blade(a, b) && (i = n)
    end
    return i
end
...
@inline function getindex(k::KVector{K,Q}, b::BitIndex{Q}) where {K,Q}
    i = to_index(k, b)
    return iszero(i) ? zero(scalar_type(k)) : (@inbounds k.data[i]) * sign(b)
end
